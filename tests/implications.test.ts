...
test("the material decides the headline treatment", () => {
  assert.equal(titles(defaultSettings)[0], "Single-shade composite bonding on 8 upper teeth");
  assert.equal(
...
test("whitening is flagged only when the target is lighter than today", () => {
  ...
  assert.ok(!titles({ ...defaultSettings, targetShade: "The same" }).includes("Discuss whitening and shade matching"));
});
...
test("shade-only does not prescribe bonding, veneers or shape changes", () => {
  const result = treatmentImplications({ ...defaultSettings, designIntent: "Shade only", treatment: "Porcelain", intensity: 100 });
  assert.equal(result.items.length, 1);
  assert.match(result.items[0].title, /Shade illustration/);
  assert.ok(!result.items.some((i) => /veneers|shape change/i.test(i.title)));
});
