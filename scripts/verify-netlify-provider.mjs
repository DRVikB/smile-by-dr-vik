...
export async function verifyNetlifyProvider(env, fetcher = globalThis.fetch) {
  ...
  if (!key) throw new Error('GEMINI_API_KEY is missing from the Netlify production build environment.');
...
if (process.argv[1]?.endsWith('verify-netlify-provider.mjs')) {
  try {
    ...
    console.error(error instanceof Error && /^(Netlify Gemini|GEMINI_API_KEY|SMILE production)/.test(error.message)
